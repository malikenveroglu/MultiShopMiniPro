...
namespace MultiShopMiniPro.Areas.Admin.Controllers
{
    ...
    public class ProductController : Controller
    {
        ...
        public async Task<IActionResult> Create(CreateProductVM productVM)
        {
            ...
            bool orderExists = await _context.Products.AnyAsync(p => p.Order == productVM.Order && !p.IsDeleted);
            ...
            bool categoryExists = await _context.Categories.AnyAsync(c => c.Id == productVM.CategoryId && !c.IsDeleted);

            if (!categoryExists)
            {
                ModelState.AddModelError(nameof(CreateProductVM.CategoryId), "Selected category does not exist");
                productVM.Categories = await GetCategoriesAsync();
                return View(productVM);
            }
        ...
        public async Task<IActionResult> Update(int? id, UpdateProductVM productVM)
        {
            ...
            bool orderExists = await _context.Products.AnyAsync(p => p.Order == productVM.Order && p.Id != id && !p.IsDeleted);
            ...
            if (!categoryExists)
            {
                ModelState.AddModelError(nameof(UpdateProductVM.CategoryId), "Selected category does not exist");
                productVM.Image = existed.Image;
                productVM.Categories = await GetCategoriesAsync();
                return View(productVM);
            }
